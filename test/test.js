...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( sger.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function performs the rank 1 operation `A = alpha*x*y^T + A`', function test( t ) {
	...
	expected = new Float32Array( [ 2.0, 4.0, 6.0, 6.0, 9.0, 12.0 ] );
	...
	expected = new Float32Array( [ 2.0, 4.0, 5.0, 8.0, 8.0, 12.0 ] );
	t.strictEqual( v, A, 'returns expected value' );
	t.strictEqual( isSameFloat32Array( getData( v ), expected ), true, 'returns expected value' );
...
tape( 'the function supports ndarrays having non-unit strides', function test( t ) {
	...
	expected = new Float32Array( [ 3.0, 6.0, 9.0, 8.0, 13.0, 18.0 ] );
	t.strictEqual( v, A, 'returns expected value' );
	t.strictEqual( isSameFloat32Array( getData( v ), expected ), true, 'returns expected value' );
	t.end();
});
